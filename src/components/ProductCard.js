...
function ProductCard(props) {
  ...
  return (
    <CardLink className="flex flex-col items-center gap-[0.25rem] md:min-w-[14rem] md:min-h-[25rem] rounded-[1rem] bg-grey-light py-[1rem] border-[inset] hover:shadow-md" to={productpath} title={title + " by " + author}>
      <img src={cover} className="mx-auto my-[0.5rem] md:w-[10.5rem] md:h-[13.25rem] w-[8rem] h-[10rem]" />
      ...
      <div className="flex flex-row mx-auto px-[1rem] gap-[0.25rem]">
        <AddToCartButton book_id={_id} />
        {isAuthenticated && <LikeButton type="icon_only" />}
      </div>
    </CardLink>
  )
}
